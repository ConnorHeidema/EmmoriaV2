	...
	// latest work here
		...
		while(dataStream >> currentToken)
		{
				...
				currentToken.back() == ':')
			{
				...
				currentSprite = PortraitMapping::GetDialogIcon(currentToken);
				text = "";
				continue;
			}
			text += currentToken + " ";
...
{
	std::list<DialogComp> finalDialogComp;
	for (auto const& dialogItem : dialogList)
	{
		finalDialogComp.emplace_back(dialogItem);
	}
